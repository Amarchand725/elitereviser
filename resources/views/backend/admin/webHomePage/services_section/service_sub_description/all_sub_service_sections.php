   ...
   /* Categories Get With Search  */
	$(function() {
		...
		function ajaxlistCategories(page_url = false)
		{
			...
			var base_url = '<?php echo base_url('all-sub-services-with-ajax/0') ?>';
			if(page_url == false) {
    ...
    function deleteData(id) {
        swal({title:"",
            ...
            if (isConfirm) {
                $.ajax({
                    type:"POST",
                    url:"<?php echo base_url('delete-sub-service/'); ?>"+id,
                    dataType:'JSON',
                    success:function(data){
                        if(data==true){
                            $('#tr-remove'+id).fadeOut(500);
                            swal({title:"Successfully!",text:"Sub service section deleted successfully",icon:"success",timer:2000});
                        }
                    },
                    error:function (er) {
                        console.log(er);
