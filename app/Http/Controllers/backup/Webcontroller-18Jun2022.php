...
class Webcontroller extends Controller
{
    ...
    public function fileUploadPost(Request $request)
    {
        $validator = Validator::make($request->all(), array(
            'file' => 'required|mimes:pdf,docx,doc,xlsx,xls,txt,ods',
...
//            return json_encode(array('status' => false, 'msg' => $validator->errors()->all()[0]));
//            return json_encode(array('msg' => $validator->errors()->all()[0]));
...
//            exit;
        ...
        if ($request->file->extension() == 'doc') {
            $fName = $request->file->getClientOriginalName();
            $request->file->move(public_path('/'), 'temp.doc');
...
//            exit;
            ...
            //Load word file
            $Content = \PhpOffice\PhpWord\IOFactory::load(public_path('temp.doc'));
...
//            exit;
        ...
        if ($request->file->extension() == 'txt') {
            $fName = $request->file->getClientOriginalName();
            $request->file->move(public_path('/'), 'temp.txt');
...
//            $Content = \PhpOffice\PhpWord\IOFactory::load(public_path('temp.xlsx'));
            $Content = \PhpOffice\PhpSpreadsheet\IOFactory::load(public_path('temp.txt'));
...
//            $writer     =\PhpOffice\PhpSpreadsheet\IOFactory::createWriter($Content, 'Tcpdf');
